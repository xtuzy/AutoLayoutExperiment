...
namespace Kiwi_Ts
{
        ...
        where T2 : new()
    {
        ...
        public List<Pair<T1, T2>> array;

        public IndexedMap(int indexCapacity, int arrayCapacity)
        {
            array = new List<Pair<T1, T2>>(arrayCapacity);
            index = new List<int>(indexCapacity);
        }

        public IndexedMap()
        {
            array  = new List<Pair<T1, T2>>();
            index = new List<int>();
        }
        ...
        */
        ...
        {
            if(this.index.Count-1 < key.id())//不存在
                return false;
        ...
        */
        public Pair<T1, T2> setDefault(T1 key, Factory<T2>? factory)
        {
            ...
            { 
                ...
                var count = index.Count-1;
                for(var i = count+1; i <= key.id(); i++)
                {
                    if (i == key.id())
                    {
                        this.index.Add(this.array.Count);

                    }
                    else
                    {
                        this.index.Add(DefaultIndexValue);//置为空
                    }
                }
                this.array.Add(pair);
                return pair;
        ...
        */
        public Pair<T1, T2> Insert(T1 key, T2 value)
        {
            ...
            if(this.index.Count-1>=key.id())//如果存在
            {
                var i = this.index[key.id()];
                if (i == DefaultIndexValue)//但为-1
                {
