...
namespace Kiwi_Ts
{
    public class Row
    {
        ...
        {
            Cells = new IndexedMap<Symbol, double>();
            Constant = constant;
        }
        ...
        public IndexedMap<Symbol, double> Cells { get; private set; }

        public double Constant { get; private set; }

        public bool IsConstant => this.Cells.empty();
        ...
        public Row copy()
        {
            var theCopy = new Row(this.Constant);
        ...
        {
            var pair = Cells.setDefault(symbol,()=>0.0);
            if (nearZero(pair.Value += coefficient))
            {
                Cells.Remove(symbol);
            }
        }
        ...
        public void Insert(Row other, double coefficient = 1.0)
        {
            ...
            {
                Insert(pair.Key, pair.Value * coefficient);
            }
        }
        ...
        {
            var pair = Cells.Remove(symbol);
            var coeff = -1.0 / pair.Value;
            Constant *= coeff;
            foreach (var pair1 in Cells)
            {
                pair1.Value *= coeff;
            }
        }
        ...
        {
            var pair = Cells.find(symbol);
            return pair != null ? pair.Value : 0.0;
        }
